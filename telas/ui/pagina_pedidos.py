...
class PaginaPedidos(QWidget):
    ...
    def adicionar_item(self, produto_id, nome, unidade, quantidade, preco_unitario, sp=False, hidro=False, nao_cobrar=False):
    ...
    def atualizar_tabela_itens(self):
        ...
        for linha, item in enumerate(self.itens_pedido):
            subtotal = 0.0 if item.get("nao_cobrar") else item["quantidade"] * item["preco_unitario"]
            total += subtotal

            nome = (
                item["nome"]
                + (" (SP)" if item.get("sp") else "")
                + (" (Hidro)" if item.get("hidro") else "")
                + (" (Não cobrado)" if item.get("nao_cobrar") else "")
            )
    ...
    def salvar_pedido(self):
        ...
        itens = [
            (
                ...
                item.get("sp", False), item.get("hidro", False), item.get("nao_cobrar", False),
            )
